...
P6-Agent-01 publishes concise Server Instructions, `sfoa://agent-playbook/current`, request-scoped `sfoa://agent-capabilities/current`, Prompt `sfoa_salesforce_assistant`, and the Tool-only fallback. `get_record_links` derives up to 50 credential-free Lightning links from the configured trusted `SFOA_LIGHTNING_BASE_URL` and performs no Salesforce API call. All guidance comes from `@sfoa/agent-playbook` version `1.1.0`; Dynamic Forms evidence is `NOT_AVAILABLE` and no Runtime Form Engine is implemented.
