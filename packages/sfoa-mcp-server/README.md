...
P7-02 creates one server-authoritative Request Audit Context for every definite `tools/call`. The context is enriched only through typed identity/route APIs and is carried across the Tool callback by a thin `AsyncLocalStorage` carrier. Optional observability headers are bounded metadata and never enter Salesforce Tool input schemas. No request collector, queue, batch writer, or Salesforce instrumentation is present.
