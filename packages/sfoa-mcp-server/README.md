...
P8-07 adds independently governed `create_records`, `update_records`,
`resolve_field_display_values` and `get_record_relationship_context`. Discovery
remains lazy. Batch rows reuse CREATE/UPDATE policy, request USER and managed
fields before one collection mutation. Audit retains one wire API call and
explicit partial/unknown counts. A `PARTIAL_SUCCESS` batch is a successful Tool
execution with an incomplete business result. Canonical guidance is Playbook 1.8.0.
...
P6-Agent-01 publishes concise Server Instructions, `sfoa://agent-playbook/current`, request-scoped `sfoa://agent-capabilities/current`, Prompt `sfoa_salesforce_assistant`, and the Tool-only fallback. `get_record_links` derives up to 50 credential-free Lightning links from the configured trusted `SFOA_LIGHTNING_BASE_URL` and performs no Salesforce API call. Guidance comes from canonical Playbook `1.8.0`; Dynamic Forms availability depends on the request's exact object policy and context result. CREATE candidates exclude Master when the current USER has an available non-Master type; Master-only creation remains supported.
