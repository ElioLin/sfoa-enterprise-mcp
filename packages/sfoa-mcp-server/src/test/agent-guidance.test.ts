...
test('MCP-native Agent guidance exposes Instructions, Resources, Prompt, fallback, and trusted links', async () => {
  ...
  try {
    ...
    assert.match(client.getInstructions() ?? '', /SFoA Salesforce Agent Playbook 1\.6\.1/u);
    ...
    assert.match(resourceText(playbook), /Playbook-Version: 1\.6\.1/u);
    ...
    assert.equal(asRecord(fallback.structuredContent).playbookVersion, '1.6.1');
