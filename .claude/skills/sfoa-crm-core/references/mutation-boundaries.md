...
Core 不定义 Required Field / Dependency / READY Gate 的完整算法。当前 CREATE / UPDATE 仍使用真实 Playbook 和 Action Context；下一阶段 `sfoa-record-change` 才专门改进字段遗漏、初始事实与 draft refinement。该 Skill 当前未实现，不影响使用现有已授权 MCP 能力。
...
返回实际成功、失败、未知与尚未完成的部分，保留错误码与必要纠正信息。一次 CREATE 必填字段遗漏是后续专业 Skill 的验收样本，不应演变为 Core 的对象专用表单。
