...
`P0 = FINAL ACCEPTED; P1 = FINAL ACCEPTED; P2 = FINAL ACCEPTED; P2-CLOSURE HOTFIX01 = PASS; P3-CLOSURE HOTFIX01 = PASS; P3-CLOSURE HOTFIX02 = PASS; P3 = FINAL ACCEPTED; P4 = FINAL ACCEPTED; P5 = FINAL ACCEPTED; P6 implementation is present on main at c849e577; P6 REAL-AGENT EVALUATION = READY / NOT STARTED; P7-01 = COMPLETE; P7-02 = COMPLETE; P7-03 = COMPLETE; P7-04 = COMPLETE; P7-05 = COMPLETE; P7-06 = COMPLETE; P7-07 = IMPLEMENTED / AWAITING MAINTAINER FINAL REVIEW; P7-08 = COMPLETE; P7-09 = COMPLETE`

Maintainer direction completed P6-ID-01 USER_BOUND credentials and P6-ID-02 Buntu identity, then authorized P6-Agent-01 and P6-DML-01. Git history confirms that implementation is now present on latest `main` commit `c849e577`; this is a repository fact and does not rewrite any historical Gate result. Maintainer review subsequently completed P7-01 through P7-06 and authorized P7-07. P7-07 is implemented awaiting Maintainer final review. The 2026-09-01 Maintainer direction explicitly authorized P7-08 as a local Agent-native Skill/toolkit under ADR-0017; P7-08 is COMPLETE following HOTFIX01, which fixed the `.gitignore` `lib` delivery defect and added the `skill:delivery` / `skill:smoke` trackability and clean-clone gates; HOTFIX02 then hardened `skill:smoke` to rebuild from committed `HEAD` bytes via `git archive` rather than working-tree files, closing the reproducibility gap.

The 2026-09-01 P7-09 lifecycle change is accepted under ADR-0018. USER and DIAGNOSTIC RequestScopes now own lazy Promise-memoized Salesforce providers. Scope/server composition, `initialize`, `tools/list`, Resources, Prompts, unchanged official `get_username`, `get_agent_playbook`, and `get_record_links` create zero Salesforce Connections; actual SOQL/DML/context/Diagnostic execution creates one fresh role-bound Connection for that request. Route resolution, Control Plane snapshot, identity sources, Tool/DML policy, P7 evidence, official Tool contracts, and Agent guidance are unchanged. No process-level token/Connection cache exists. P7-09 HOTFIX01 (2026-09-02) replaced the implicit `hostOwnedArguments.includes('usernameOrAlias')` Connection heuristic with an explicit `requiresSalesforceConnection` boolean on each remote Tool contract, aligned the DML lazy-auth failure onto the DML output contract, and pinned Tool `durationMs` to end-to-end latency including lazy auth/connection.

P6-Agent-01/P6-DML-01 add no business-object workflow, prompt database/editor/history, generic default/constant engine, permission replica, Dynamic Forms evaluator, runtime form engine, DELETE, managed-field cache, or identity override. Dynamic Forms evidence is `NOT_AVAILABLE`. Strict `PLATFORM_USER_LOOKUP` and CREATE-only `AI_CREATED_MARKER=true` retain server precedence. The opt-in CREATE-only `PLATFORM_USER_LOOKUP_FALLBACK` preserves explicit client values and otherwise reuses the platform Lookup; migrations 010/011 preserve existing rows and forbid fallback UPDATE scope. Playbook 1.5.1 distinguishes required CREATE clarification from optional omission and retains minimum UPDATE mutation. `get_record_links` accepts no host and makes no Salesforce API call; it uses only the configured credential-free HTTPS origin `SFOA_LIGHTNING_BASE_URL` and never falls back to `Connection.instanceUrl`. Pre-dispatch managed-field failures are `FAILED`; `MCP_DML_OUTCOME_UNKNOWN` remains post-dispatch and non-auto-retry across every distribution surface.
...
| Payload capture could confuse a logical Tool result with transport delivery | A generated success could be misreported as delivered after client disconnect | P7-06 retains logical `responseSummary` separately, records actual MCP wire prefix, and marks `finish` only as `RESPONSE_FINISHED` with `clientReceiptConfirmed=false`; close-before-finish is `CLIENT_DISCONNECTED` |
| A future process-global current Audit/Collector could cross users under concurrency | Any cross-Audit leak is a blocking security defect | P7-02 uses request-bound Context and passed 100-way zero-leak; P7-03 must retain this boundary and pass the 50/100/200 full-pipeline Gates |
...
| Asynchronous Audit persistence may fail or saturate resources | Snapshot loss is possible under Queue Full/DB failure, but Tool results must not change | Capacity-1000 non-blocking Queue, background retry/drop, dedicated two-connection pool, health metrics, and fail-open Gates are mandatory |
...
| P7-E2E-AUDIT-BL-1.2 | 2026-08-30 | Implemented P7-02 server-authoritative Request Audit Context, controlled identity/route enrichment, thin AsyncLocalStorage carrier, one-context/one-Audit-Call linkage, bounded optional client metadata, and a 100-way Promise-interleaving zero-leak Gate; P7-03 remains unstarted. |
| P7-E2E-AUDIT-BL-1.3 | 2026-08-30 | Recorded Maintainer completion of P7-01/P7-02 and implemented P7-03 request-bound collection, explicit terminal authority, immutable bounded Snapshot, capacity-1000 non-blocking Queue, batch-50 background Writer, two-connection Audit pool, bounded retry/shutdown, health metrics, and 50/100/200 isolation/fail-open/performance Gates. P7-04 remains unstarted. |
| P7-SF-API-AUDIT-BL-1.0 | 2026-08-31 | Recorded Maintainer completion of P7-03 and implemented P7-04 per-wire-attempt JSforce auditing, deterministic API classification, Purpose scopes, bounded critical-failure preservation, migration 006, same-pipeline asynchronous persistence, retry/duplicate/failure/OAuth/50-100-200 MySQL and paired performance Gates. P7-05 remains unstarted. |
...
| P7-LAZY-CONNECTION-BL-1.0 | 2026-09-01 | Completed P7-09 with one request-owned Promise-memoized Salesforce resource, route-only OrgService, delayed live API-version workspace configuration, lazy DML/official/context/Diagnostic acquisition, zero local/protocol Salesforce calls, exact one-Connection remote call-count and concurrent/failure/cleanup regression Gates. No Tool contract, identity/governance/Audit semantic, dependency, migration, cache/pool, Agent guidance, Admin UI, or official Salesforce TypeScript change. |
| P7-LAZY-CONNECTION-HOTFIX01 | 2026-09-02 | Closure HOTFIX: explicit `requiresSalesforceConnection` boolean replaces the `hostOwnedArguments.includes('usernameOrAlias')` Connection heuristic (get_username=false, run_soql_query/retrieve_metadata=true), enforced by an upstream-drift guard; DML lazy-auth failure aligned to the DML output contract; Tool `durationMs` pinned as end-to-end latency including lazy auth/connection. Live P4 Diagnostic PASS; P2/P3 env validators NOT RE-RUN under the mysql+BUNTU checkout. |
| MANAGED-LOOKUP-FALLBACK-1.0 | 2026-09-05 | Add explicit-user-first platform Lookup strategy, compatible ENUM/CHECK migration 010, safe capabilities and Playbook 1.5.0; preserve strict strategies, request identity, Salesforce authority, and audit summaries. |
| MANAGED-LOOKUP-FALLBACK-HOTFIX01 | 2026-09-05 | Require CREATE-only fallback with migration 011 and Playbook 1.5.1; reject duplicate client aliases before lookup and align Action Context object casing. Preserve user deployment history and strict strategies. |
