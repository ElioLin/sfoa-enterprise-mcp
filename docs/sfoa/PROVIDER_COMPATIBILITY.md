...
`PASS` in this matrix means version resolution, build/linkage, Provider registration, and the applicable local transport regression pass. It does not claim the separate live SFoA JWT/SOQL/Metadata Gates; those remain `NOT TESTED` until `.env.local` is supplied.
...
| `@salesforce/core` | Root override / package ranges resolve to 8.29.0 | 8.29.0 in official host, HTTP POC, and Closure Harness | 8.29.0 | PASS — LOCAL LINKAGE | Closure JWT and Connection code imports this package directly; it never spawns `sf`. Live Connection/identity remains a separate pending Gate. |
