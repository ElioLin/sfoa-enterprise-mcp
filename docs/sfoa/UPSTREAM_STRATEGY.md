...
| `package.json` | Expose the Maintainer-required P5 lifecycle, P6 Agent artifacts, and P7-08 local Skill/diagnostic commands at the repository root | Added scripts only, including `agent:*`, `skill:*`, and `ai:*`; retained the existing Yarn Classic `packages/*` workspace and `nohoist` policy | Require contributors to remember long workspace-specific or direct Node commands, or add another task-runner dependency; rejected because the small standard-library commands are clearer and dependency-free | LOW |
